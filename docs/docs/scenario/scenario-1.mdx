...
For this, we use Coohom, which is an interior designer tool that allows to render images of the room in 3D. Of course, there are alternatives, such as Planner 5D, Sweet Home 3D, ... .
...
There, we search for curtain and place it in front of the window. For later rendering, it's recommended to hide the curtain for now.
...
Once we're finished, we perform the rendering. For that, we go to 'Render' mode and generate a picture of each wall - note that the curtain, heater and tablet should be hidden during this phase. As a result, we get four pictures, one of each wall without the curtain, heater and tablet.
...
# Creating the first scenario within the study platform

To create the first scenario, we modify the game configuration file `platform\src\game.json`.
...
We define here that the in-game time starts at 8 o'clock and every second in real-time corresponds to 10 seconds of in-game time.
...
Now, we can define the devices. We define devices within walls:
```json
{
   "image": "assets/images/room/wall1/wall1.webp",
   "default": true,
...
Next, we create the heater device with an boolean interaction `Power` in the same way as the curtain
...
Since we have finished creating the devices, we can proceed to creating the rules
...
Here, we define the ID, description, and preconditions which can be based on Device variables (through device id and interaction id), time or context (environment variables, task or user variables of base64 encoded JSON). If all preconditions apply (using `AND`), then the action(s) will be triggered.
...
Since we have finished creating the rules, we can proceed with the explanation engine.
...
Next, we can set the environment variables which are shown below the game in a bar but is optional. For informational purposes, it should show that the outdoor temperature is 10 °C. Thus, the environment is
```json
{
    "environment": [
...
Next, we set the default device properties which is also optional in our case since we have only one base task. Setting the default device properties is useful in settings where the user may have interacted with a device before (in a previous task) and that conflicts with future task's goals.
...
Here, we use the device ID again and the condition with the property and operator that needs to be fulfilled to finish the task and thus complete the game. Note, that we can define multiple task goals of which all conditions must be satisfied.

## Game / Explanation JSON

Since we have finished creating the scenario, we can restart the development server via `npm run dev` and create a new session via http://localhost:3000/?data=eyB9.
