...
Devices are interactive elements that can be placed on walls within the game environment. Each wall can contain multiple devices, but a device can only be placed on a single wall. Participants can interact with these devices through various interaction types. A device is defined by its core properties including name, unique identifier, interaction types, and visual states that control its appearance.
...
Each device has two identification properties:

- **`name`**: Human-readable display name of the device (e.g., "Deep Fryer", "Coffee Machine")
- **`id`**: Unique identifier used for referencing the device in rules, tasks, and game logic (e.g., "deep_fryer", "coffee_machine")
...
:::tip Device Naming
Use descriptive names for display purposes and consistent, lowercase identifiers with underscores for IDs. The ID should be unique across all devices in your game configuration.
:::

The `id` property is essential for:
- **Rules**: Referencing devices in preconditions and actions
- **Tasks**: Specifying devices in goals and default properties
- **Game Logic**: Internal device state management and references
