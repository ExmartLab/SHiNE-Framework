...
Devices are interactive elements that can be placed on walls within the game environment. Each wall can contain multiple devices, and participants can interact with these devices through various interaction types. A device is defined by its core properties including name, interaction types, and visual states that control its appearance.
