...
```
Timer/Goal Trigger → Task Handler → Database Update → Next Task Setup
                                                            │
                                                            ▼
Device State Changes → Rule Evaluation → Real-time Updates → Frontend Sync
```

### 🚀 Session Lifecycle

```
Create Session API → MongoDB Collections Init → Game Data API → Frontend Load
                                                                        │
                                                                        ▼
Socket Connection → Real-time Interactions → Study Completion → Data Export
```
