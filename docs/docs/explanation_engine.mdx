...
1.  **Integrated Engine (Default)**    
    Use the built-in explanation logic that is tightly coupled with the simulation environment. This engine automatically monitors interactions and generates context-aware explanations based on the pre-defined rules and states.
...
|----------------------------------|-----------|-------------|
| `explanation_trigger`            | `string`  | Determines when explanations are shown. Options: `"on_demand"` or `"automatic"`. |
| `explanation_engine`             | `string`  | Selects the explanation system. Options: `"integrated"` or `"api"`. |
| `explanation_rating`             | `string`  | Specifies how users rate explanations. Options: `"like"`, `"stars"`, or `"none"`. |
| `allow_user_message`             | `boolean` | Allows participants to send custom free-text messages (e.g., questions or reactions). |
...
Use **on_demand** when you want participants to discover issues independently before seeking help. Use **automatic** for educational scenarios where immediate feedback is beneficial.
:::
...
Enables **interactive communication** between participants and the **external explanation engine**, allowing participants to express what they specifically want to understand.
```json
{
    "allow_user_message": true
}
```
When enabled, participants are given the option to **submit custom free-text queries** (e.g., “Why can’t I turn on the oven?” or “Why did the light switch off now?”). This makes it possible to collect more precise user intents and supports generation of personalized explanations,  
if the external engine supports such functionality. This feature can be especially useful in studies focused on:
-   User explanation preferences  
-   Natural language interaction in explainable systems    
-   Real-time adaptive feedback
    
:::info
 The ability to process these messages and generate tailored explanations depends entirely on the **implementation of the external explanation engine**.

The framework merely acts as a middleware, collecting messages and forwarding them to the explanation system, then delivering the response back to the participant.
:::
...
This design allows researchers to precisely specify:
	...
	•	Under which conditions it is relevant
...
Showing the explanation is governed separately by the explanation_trigger setting (e.g., on_demand or automatic).
:::
...
```
This rule disables the coffee machine after 5 cups and generates the explanation "coffee_machine_01" (see last example above) that is  to explain the action.
...
The property is `external_engine_type` **required** when the explanation_engine is set to "external".
It defines the communication protocol used to connect to the external explanation engine.
...
```
>💡 You can replace "on_demand" with "automatic" based on your study needs.
Both "like" ratings and allow_user_message can be enabled or disabled independently.
...
This endpoint is called **when an explanation is requested**, either **on-demand** (e.g., user clicks “Explain Me!”) or **automatically** (based on rule triggers and system configuration).
...
**Request with User Message (if allow_user_message: true):**
```json
{
    "user_id": "64bdb062-cb25-487f-8373-c56ac18fba5a",
...
> -   **enforce_automatic_explanation**: Optional boolean.    
>     -   If true, the explanation is shown immediately—even if the system is set to on_demand.
