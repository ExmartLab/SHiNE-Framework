...
We begin by configuring the main simulation setup in 'platform/src/game.json' (See [Game Config Schema](../game-config/game_schema/game_config_schema)).
...
#### 🪟 Curtain 
We create the first device - curtain - as follow: 
```json
{
    "name": "Curtain",
    "id": "curtain",
...
- **Rule 1**: Keep Heater On if Window(curtain) is Open and Temp < 20°C
- **Rule 2**: Raise Temperature to 21°C When Curtain is Closed
...
via the 'explanation.json' file, located in the same directory as 'game.json'. For a full description of the explanation configuration schema, see [Explanation Schema Documentation](../game-config/exp_schema/explanation_engine).
...
- The curtain to Open (the 'Closed' property is set to 'false')
