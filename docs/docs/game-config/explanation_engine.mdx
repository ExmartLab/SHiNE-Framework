...
To create new rule with associated explanation, a JSON object need to be defined in the array `integrated_explanation_engine` as follow

- name: The name of the Rule
- explanation: Explanation to be shown to the user
- user_condition: To which condition, the explanation is shown
- condition: Conditions of when to trigger the rule and explanation, relating to the device's variables or in-game time
- action: The action that is fired when all conditions apply, such as modifying variables
