...
Each device in the simulation can expose **interactions** , these are the ways participants can observe or manipulate the device’s state. 
For example:
...
-  **Stateless Actions** – trigger-like interactions with no memory (e.g., “Start”, “Reset”)
...
Visual states can optionally override a device’s default position properties to accommodate visual transformations that occur in different states, 
