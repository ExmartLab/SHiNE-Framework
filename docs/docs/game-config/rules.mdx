...
1. **ID** (optistring, optionaonal): A unique identifier for the rule
2. **Name** (string, required): A human-readable name describing the rule's purpose (e.g., "Turn Off Coffee Machine Automatically").
3. **Preconditions** (array, required): An array of condition objects that represent the _"IF"_ part of a rule. Preconditions specify when a rule becomes eligible for execution. 
All listed preconditions must be met (logical AND). They can evaluate:
   - Device State Conditions
   - Time conditions
   - Contextual Conditions
   - Task Conditions
4. **Delay** (number, optional): The time in seconds to wait after the preconditions are met before executing the action. 
This allows for the modeling of system latency or the creation of more naturalistic, timed behaviors.
5. **Actions** (array, required): An array of action objects that represent the _"THEN"_ part of a rule. 
