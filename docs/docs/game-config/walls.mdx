...
| `devices`  | array    | Array of devices to be placed on this wall. Each device is defined using the [Device Schema](./deviceSchema.md) |
| `doors`    | array    | Array of doors to be rendered on this wall. Each door is defined using the [Door Schema](./doorSchema.md) |
...
### Complete Wall Configuration Examples
...
## Default Wall Configuration (DONE)
...
:::warning Door Destination Validation (put is right place)
Always ensure that:
...
### Wall with Multiple Devices (done)
...
### Wall with Door Navigation (DONE)
...
### Wall with Both Devices and Doors (done)
